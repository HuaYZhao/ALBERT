...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu,
    ...
    grads = [g * 1000 if v.name.startswith("slqa") or v.name.startswith("co-attention") else g for v, g in zip(tvars, grads)]
...
def create_bert_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu,
    ...
    tvars = [v for v in tf.trainable_variables() if v.name.startswith("bert")]
    grads = tf.gradients(loss, tvars)
...
def create_other_optimizer(loss, init_lr, num_train_steps, use_tpu,
                           optimizer="adamw", poly_power=1.0, start_warmup_step=0):
    """Creates an optimizer training op."""
    global_step = tf.train.get_or_create_global_step()
    ...
    if use_tpu:
        optimizer = contrib_tpu.CrossShardOptimizer(optimizer)
    tvars = [v for v in tf.trainable_variables() if not v.name.startswith("bert")]
    grads = tf.gradients(loss, tvars)
