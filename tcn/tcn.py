...
def TemporalConvNet(input_layer, num_channels, kernel_size=2,
                    dropout=0., init=False, atten=False, use_highway=False,
                    use_gated=False):
    ...
    for i in range(num_levels):
        ...
        input_layer = TemporalBlock(input_layer, out_channels, kernel_size, stride=1, dilation_rate=dilation_size,
                                    counters=counters, dropout=tf.constant(dropout, dtype=tf.float32), init=init,
                                    atten=atten, use_highway=use_highway, gated=use_gated)
