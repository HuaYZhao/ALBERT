...
def main(_):
    ...
    # from albert_tf2.albert import AlbertConfig
    # albert_config = AlbertConfig.from_json_file(FLAGS.albert_config_file)
    # import bert
    # albert_config = bert.params_from_pretrained_ckpt(os.path.dirname(FLAGS.albert_config_file))
    ...
    if FLAGS.do_predict:
        ...
        if tf.gfile.Exists(checkpoint_path + ".index"):
            ...
            best_perf = np.mean([result[0][key] for key in key_name.split(',')])
            ...
            if not steps_and_files:
                ...
                for ele in sorted(steps_and_files.items()):
                    ...
                    for key in sorted(result.keys()):
                        ...
                        writer.write("%s = %s\n" % (key, str(result[key])))
                    if np.mean([result[key] for key in key_name.split(',')]) > best_perf:
                        best_perf = np.mean([result[key] for key in key_name.split(',')])
                        for ext in ["meta", "data-00000-of-00001", "index"]:
                            src_ckpt = checkpoint_path + ".{}".format(ext)
