...
def v2_model_fn_builder(albert_config, init_checkpoint, learning_rate,
                        ...
                        end_n_top, dropout_prob, hub_module, use_adv=False):
    """Returns `model_fn` closure for TPUEstimator."""
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            if use_adv:
                tf.logging.info("using adversarial training.")
                ...
                embedded = embedded_inputs[0]
                tf.logging.info("embedded: {}".format(embedded))
                grad, = tf.gradients(
                    total_loss,
                    embedded)
                grad = tf.stop_gradient(grad)
                perturb = _scale_l2(grad, 0.125)  # set low for tpu mode
                embedded_inputs = embedded + perturb
