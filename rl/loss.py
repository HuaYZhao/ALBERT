...
from rl.utils import mask_to_start, tf_f1_score


def cross_entropy_loss(logits, answer_start, answer_end, loop):
    """
    Cross entropy loss across all decoder timesteps
    """
    logits = logits.concat()
    if len(answer_start.shape) == 1:
        answer_start = tf.reshape(answer_start, [-1, 1])
    if len(answer_end.shape) == 1:
        answer_end = tf.reshape(answer_end, [-1, 1])
    answer_span = tf.concat(
        [answer_start, answer_end], axis=1)
    answer_repeated = tf.tile(answer_span, (loop, 1))
    ...
    start_loss = tf.stack(tf.split(start_loss, loop), axis=1)
    end_loss = tf.stack(tf.split(end_loss, loop), axis=1)
