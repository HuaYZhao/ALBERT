...
def f1_score(prediction: Counter, ground_truth: Counter):
    """
    From the official evaluation script for the SQuAD dataset
    """
    # prediction_tokens = normalize_answer(prediction).split()
    # ground_truth_tokens = normalize_answer(ground_truth).split()
    common = prediction & ground_truth
    num_same = sum(common.values())
    if num_same == 0:
        return 0.0
    precision = 1.0 * num_same / sum(prediction.values())
    recall = 1.0 * num_same / sum(ground_truth.values())
...
def tf_f1_score(tensors):
    ...
    return tf.cast(tf.py_func(f1_score, [prediction, ground_truth], tf.float64, stateful=False),
                   tf.float32)
