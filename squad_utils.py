...
def create_v2_model(albert_config, is_training, input_ids, input_mask,
    ...
    p_mask = tf.cast(features["p_mask"], dtype=tf.float16)

    # logit of the start position
    with tf.variable_scope("start_logits", dtype=tf.float16):
    ...
    with tf.variable_scope("end_logits", dtype=tf.float16):
        if is_training:
            ...
            start_index = tf.one_hot(start_top_index,
                                     depth=max_seq_length, axis=-1, dtype=tf.float16)
    ...
    with tf.variable_scope("answer_class", dtype=tf.float16):
        # get the representation of CLS
        cls_index = tf.one_hot(tf.zeros([bsz], dtype=tf.int32),
                               max_seq_length,
                               axis=-1, dtype=tf.float16)
